...
func main() {
	...
	wifC, _ := network["wsp"].CreatePrivateKey()
	wifD, _ := network["wsp"].GetDecompressedPrivateKey(wifC.PrivKey)
	addressC, _ := network["wsp"].GetAddress(wifC)
	addressD, _ := network["wsp"].GetDecompressedAddress(wifC)
